...
public class MethodologyController {
    ...
    @PreAuthorize("hasAnyRole({'CENTERO_ADMIN'})")
    @GetMapping("/find-all")
    ...
    public ResponseEntity<ApiResponse<Void>> findOne(@PathVariable("id") Long id) {
        return ApiResponse.ok("fetch one user info  " + id);
    }

    @PreAuthorize("hasRole('CENTERO_USER')")
    @GetMapping("/model-attribute-valid")
    public ResponseEntity<ApiResponse<Void>> modelAttributeValid(@Valid MethodologyDto modelDto) {
    ...
    public ResponseEntity<ApiResponse<Void>> methodArgumentValid(@Valid @RequestBody MethodologyDto modelDto) {
